...
class DQN2(object):
    ...
    def _generate_model(self):
        ...
        self.a1 = tf.placeholder(tf.float32, shape=(None, self.a_dim), name='a1')
        ...
        q_target = self.r + self.gamma \
            * (tf.reduce_max(self.q_target_z_a1, axis=1, keepdims=True)+tf.reduce_max(self.q_target_z_a2, axis=1, keepdims=True)) * (1 - self.done)

        q_eval = tf.reduce_sum(self.a1 * self.q_eval_z_a1 + self.a2 * self.q_eval_z_a2, axis=1, keepdims=True)
